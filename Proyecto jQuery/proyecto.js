...
function buscar_peli(){
    ...
    numero_pelis = 0;
    pag = 1;
    pagina = "http://www.omdbapi.com/?s=" + titulo + "&" + "type=" + tipo + "&page=" + pag + "&apikey=6040d029";

    /* Realizamos la petición Ajax al pulsar enter o clicar en buscar*/
    $.ajax({
        url: pagina
    })
    .done(function(data){
        ...
        $("#enlace_1").remove();
        $("#enlace_2").remove();
        // Añadimos el titulo de la película a Home.
        ...
        if (data.Response == "True"){
            ...
            if(tipo == "movie"){
                no_encontrado = "<div class='no_encontrado'> No se han encontrado peliculas con el nombre indicado." +
                titulo + "</div>"
            }else if(tipo == "series"){
                no_encontrado = "<div class='no_encontrado'> No se han encontrado series con el nombre indicado." +
                titulo + "</div>"
            }else if(tipo == "episodie"){
                no_encontrado = "<div class='no_encontrado'> No se han encontrado episodios con el nombre indicado." +
                titulo + "</div>"
            }else if(tipo == ""){
                no_encontrado = "<div class='no_encontrado'> No se han encontrado peliculas, series o episodios con el nombre indicado. " +
                titulo + "</div>"
            }
...
function ver_detalles(a){
    ...
    console.log(a.value);
    id_peli = a.value;
    pagina = "http://www.omdbapi.com/?i=" + id_peli + "&apikey=6040d029";
...
// ¿Final del documento alcanzado?
    if ($(document).height() - $(window).height() == $(window).scrollTop() &! fin &! en_curso &! parar) {
        ...
        $.ajax({
            url: pagina = "http://www.omdbapi.com/?s=" + titulo + "&" + "type=" + tipo + "&page=" + pag + "&apikey=6040d029",
